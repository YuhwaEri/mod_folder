...
if(!empty($files))
{
	foreach ($files as $file) {
		...
		if(!empty($fileInfo) && $fileInfo[0] != '.' && $fileInfo[0] != '..'){
			...
			if(!empty($folderInfo)){
				// echo "han hai";
				$cm = get_coursemodule_from_id('resource', $id, 0, true, MUST_EXIST);
				 // print_r($cm);
				$context = context_module::instance($cm->id, MUST_EXIST);

				$folder = $DB->get_record('resource', array('id'=>$cm->instance), '*', MUST_EXIST);
				...
				$PAGE->set_url('/mod/resouce/edit.php', array('id' => $cm->id));
